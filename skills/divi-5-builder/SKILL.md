...
- **Grid Editor offsets use the 5.9.0 `offsetValues` map**: when `module.decoration.layout.<breakpoint>.value.display` is `"grid"`, write `gridOffsetRules: {"rules":[{"targetOffset":"first-child","offsetValues":{"columnStart":"2","columnSpan":"2"}}]}`. Do not author the legacy `offsetRule` / `offsetValue` pair; Divi 5.9.0 migrates that shape on import, frontend migration, and VB raw-content load, but new content and presets should use the map. Allowed keys, number-variable behavior, selector targeting, and preset migration details are in [module-formats.md → Grid layout and offset rules](references/module-formats.md#grid-layout-and-offset-rules-divi-590-source-verified-2026-07-29).
- **Post Filter must target a real loop ID**: set `module.advanced.targetLoop.desktop.value` on `divi/post-filter` to the exact `module.advanced.loop.desktop.value.loopId` of the Loop Builder container. Filter state uses `loop_filter-{loopId}[...]` GET parameters; an empty target or `main_query` does not produce filter parameters. See [module-formats.md → Post Filter](references/module-formats.md#post-filter-divi-510-source-verified-2026-08-10).
- **Divi 5.11 module boundaries are per-module**: `divi/charts` is a runtime-qualified Free primitive; `divi/gravity-forms`, `divi/imagely-gallery`, and `divi/payment-button` remain Pro and schema-only until the exact provider/payment environment is separately proven. Preserve provider/payment boundaries and do not infer hosted-AI authoring from module registration. See [module-formats.md → Divi 5.11 module boundaries](references/module-formats.md#divi-511-module-boundaries).
...
- **Nested Tooltip hover binding** *(failed on Divi 5.8.0; fixed for the exact qualified shapes on Divi 5.10.1, verified 2026-08-14)*: Divi 5.8.0 saved and rendered `divi/tooltip` nested under `divi/button` or `divi/contact-field`, but its local frontend runtime left those nested surfaces offscreen and did not open them on hover/click. On Divi 5.10.1, the canonical hover-trigger Tooltip nested directly under a Button and the canonical hover-trigger Tooltip nested directly under a Contact Field each opened on real pointer hover and closed on leave, on both initial load and full reload; a standalone always-open Tooltip remained the positive control. Those exact Button/Contact Field hover shapes are allowed on the tested Divi 5.10.1 release. Re-verify them before relying on a later Divi release. Do not extrapolate this result to other owner modules, click/focus or other trigger modes, pre-5.10.1 releases, or malformed/non-canonical nesting.
