...
Local attrs normally override group-preset attrs, which override module-preset attrs. An implicit group default participates at the group layer, not below an explicit module preset; see [Cascade order](#cascade-order). Whether inline attrs are present at all after a VB preset-apply action is a **separate, behavior-dependent question** that we have NOT fully characterized.
...
**Implication for diviops authoring**: attaching a preset does not imply permission to remove local settings. Preserve approved exceptions; deliberate adoption requires review of the exact overlapping style paths, including responsive/hover states. Do not depend on VB's strip-on-apply behavior. `diviops_preset_reassign` with `strip_inline=true` removes strictly equal leaves under the guards below, not differing overrides. `module_update` stores `null` as a value, not an unset instruction; see [Consolidation workflow](#consolidation-workflow) for the current removal limit.
...
### Cascade order
For matching stored paths, local attrs override group-preset attrs, which override module-preset attrs. A bucket default can supply its layer when no explicit binding is present: **an implicit group default can override an explicit module preset**. Do not place every default below every explicit preset. Computed CSS and responsive/state behavior still require verification for the selected module and property.
...
- `POST /diviops/v1/preset/reassign` — Rewrite preset refs across pages from `old_uuid` → `new_uuid`. Covers both ref types: **module-level** (`attrs.modulePreset[...]`, stacked array) and **attribute-level** (`attrs.groupPreset.<slot>.presetId`). For group-bucket swaps, also rewrites **registry chain refs** (module-bucket `groupPresets.<slot>.presetId`; group-bucket `attrs.groupPreset.<slot>.presetId`). The `scope` param controls which ref types are walked:
  ...
  - `strip_inline: true` (default) — removes **strictly equal leaves at matching paths**, preserving differing values, unrelated branches and top-level reserved identity/binding keys. Module scope compares from the block root only when the post-swap `modulePreset` stack contains just `new_uuid`. Group scope resolves each target module/slot through Divi's `GlobalPresetItemGroup`, combining mapped `styleAttrs`, `attrs` and `renderAttrs`, and applies the same singular-stack guard per slot. Stacks retain inline values (`strip_skipped: "stacked_presets_present"`). Unresolvable slots retain inline values (`strip_skipped: "slot_unresolvable"`) and emit `summary.strip_advisory_per_slot["<module>::<slot>"]`; UUID swaps and resolvable neighboring slots still proceed. Set `strip_inline: false` for reference swaps only. This is equality-based deduplication, not a semantic design-only filter: inspect the preset bags and dry-run so matching content, behavior or other unapproved properties are not removed.
...
Choose the intent before changing bindings: **attach and preserve exceptions** retains differing local values; **adopt the preset's design** removes only approved overlapping style overrides; **extract a reusable style** creates a preset only when no suitable compatible preset exists, excluding instance content, links, identity and behavior. Review a per-property keep/remove decision, preserving unrelated styles and intentional responsive/state exceptions. Strict equality is not visual similarity: a literal and a variable reference that resolve to the same color are not interchangeable bindings.

**Equal to the target preset does not mean redundant in the effective design.** Inspect applicable group/default values before removing local styling. If a required local value protects against a different inherited value, use `strip_inline: false` for the reference swap and retain it; review any other removals separately through the existing subtree-replacement limits below. Reassignment has no per-property stripping allowlist and its dry-run does not prove visual equivalence. Do not change a shared default to make a local adoption pass.

**Bounded Divi 5.13 proof (2026-09-17, Free 1.5.24):** a Button module preset and local attrs both supplied `6px` corners. Equal-leaf stripping exposed the site's implicit Button group default of `0px`; the module-preset CSS still correctly emitted `6px`, but the group/default and merged instance CSS won. Retaining the local radius restored the intended corners on desktop and phone and survived native Builder saving. Heading/Button shared color updates, intentional phone/hover exceptions and native text edit/save/reopen also passed. This is a demonstrated default-precedence interaction, not a radius-emitter defect or general lossless-conversion guarantee. The retained radius remains instance-owned and will mask later preset radius changes.
...
2. Select an existing compatible preset; only if none fits, use `diviops_preset_create` with the agreed design-only attrs. Use the selected UUID (or returned `preset.id`) as `new_uuid`.
...
4. After approval, `diviops_preset_reassign` with `mode: "apply"` rewrites the scoped refs and eligible equal inline leaves via `parse_blocks` + `serialize_blocks`. Group swaps also rewrite registry chain refs; review that shared impact even when page IDs are bounded. Read back bindings and preserved exceptions, then verify responsive/hover appearance and native VB save/reopen within the approved proof.

**Purely-inline modules (no explicit preset UUID)** use existing module updates, not `old_uuid` reassignment; implicit defaults may still apply. There is no automatic batch converter. The manual workflow:

1. `diviops_page_get_layout` / `diviops_module_get` — identify exact modules and read their current attrs; inspect the candidate preset and agree which properties remain local.
2. Reuse a compatible preset, or create one from only the approved design settings if needed; never copy the entire seed module's attrs indiscriminately.
3. For each selected module, dry-run `diviops_module_update` with `attrs: { modulePreset: ["<preset.id>"] }`, or the canonical `groupPreset` slot paths above. Apply only within the approved scope with backup/readback. Retained inline values can mask later preset edits, even when equal today; report them rather than claiming full adoption.

**Exact-removal tool gap (source-checked):** `module_update` assigns each supplied path's value; `null` remains a stored key, and there is no dedicated unset-path parameter or atomic bind-and-unset operation. It can replace a supplied subtree, but omitting a child from that replacement is broader than leaf deletion: it requires a fresh complete subtree, preservation of every unrelated/content/behavior value and separate review of the replacement. Do not use null or an empty parent as a cleanup shortcut. If the approved adoption requires exact removal without a broader replacement, stop and report the gap rather than inventing a batch converter or claiming the preset owns the property.
...
- **Inspect before reuse:** names and manifests are discovery hints. Discover IDs with storage-only `diviops_preset_audit_storage` / `diviops_preset_list`, then call `diviops_preset_inspect` for the selected preset's type/slot, stored styles, storage ambiguity and known consumers. Join its direct `variable_references.ids` to `diviops_variable_list` for actual types and values; absent IDs remain unresolved. Read `coverage`: excluded surfaces and implicit/inherited usage mean zero references is never deletion approval. Older inspectors without these fields have unknown coverage. Do not run full `diviops_preset_audit` per row or search. Prefer exact-ID reuse or an explicit local override; never silently change shared defaults. `prefix` filters stored IDs, not labels. Recheck relevant values before a later write; changed token values need not change serialized preset references or page checksums. Stored inspection is not rendered-style verification.
...
When a module consumes a preset, **avoid duplicating values that the effective preset cascade already supplies.** A matching module-preset value alone is insufficient; an implicit group default may override it.
...
- **Required precedence exception** against a conflicting group/default value, even when the local value equals the module preset (see [Consolidation workflow](#consolidation-workflow)). Keep it explicitly instance-owned rather than claiming full preset control.

Why this matters: duplicating a preset's value inline doesn't break rendering, but it **fragments the source of truth**. A future preset edit (e.g. swapping `gcid-oa-neutral-950` for a new background token across the design system) won't propagate to instances that have inline overrides — you have to hunt them down in page markup. Prefer the preset as the source for properties it effectively controls, and report the exceptions above rather than stripping them blindly.
